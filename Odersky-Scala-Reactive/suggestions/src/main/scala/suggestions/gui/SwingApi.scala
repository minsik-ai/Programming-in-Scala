  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = {
      Observable.create[String](observer => {
        log("textValue observable subbed")
        val textChange: Reaction = {
          case ValueChanged(value) => {
            log("Text Value Changed! : " + field.text)
            observer.onNext(field.text)
          }
        }
        // Event to Unit partial callback

        field.subscribe(textChange)

        Subscription(field.unsubscribe(textChange))
      })
    }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] =
      Observable.create[Button](observer => {
        log("buttonClick observable subbed")
        val buttonClick: Reaction = {
        ...
        button.subscribe(buttonClick)

        Subscription(button.unsubscribe(buttonClick))
      })
  }
