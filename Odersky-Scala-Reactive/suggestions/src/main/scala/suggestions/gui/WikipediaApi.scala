...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] =
      obs.map(t => Success(t)).onErrorReturn(e => Failure(e))               // TODO : Test if order is correct
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val timer: Subject[T] = Subject[T]()              // Subject[T] for creation of Objervable[T]
      Future {
        Thread.sleep(totalSec * 1000)
      }.onComplete{
        case Failure(e) => { timer.onError(e) }
        case Success(u) => { timer.onCompleted() }      //Future is Unit, no onNext for timer Observable.
      }
      obs.merge(timer)
    }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] =
      obs.flatMap(requestMethod).recovered                                  // TODO : Test
  }
