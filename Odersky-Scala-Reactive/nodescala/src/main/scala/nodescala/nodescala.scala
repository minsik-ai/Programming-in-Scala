...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listenerSubscription = listener.start()     // httpListener

    val respondSubscription =
      Future.run()(ct => Future {
        while (ct.nonCancelled) {
          val req:Future[(Request, Exchange)] = listener.nextRequest()
          //TODO : Shouldn't this be blocking future? seems like it would run too fast in repeat manner
          req onSuccess {
            case (request, exchange) => respond(exchange, ct, handler(request))      // response loop, respond blocks.
          }
        }
      })
